...
export default function BlinkIDPage() {
  ...
  const handleRectoCapture = (file, previewUrl) => {
    setRectoFile(file);
  ...
  const handleVersoCapture = (file, previewUrl) => {
    setVersoFile(file);
  ...
  const fileToImageFrame = async (file) => {
    try {
  ...
  const processID = async (frontImage, backImage) => {
    let sdk;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        ...
        <meta name="description" content="Vérification de pièce d'identité" />
      </Head>
